...
// WriteDirManifest writes the given directory manifest as a repository object and returns its object ID
func WriteDirManifest(ctx context.Context, rep repo.RepositoryWriter, dirRelativePath string, dirManifest *snapshot.DirManifest, metadataComp compression.Name) (object.ID, error) {
	return writeDirManifest(ctx, rep, dirRelativePath, dirManifest, metadataComp)
}
